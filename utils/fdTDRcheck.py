...
# 校正 TWT 的同时，反向标定（校正）声波测井与密度测井曲线。
...
def robust_load_tdr_txt(path, min_cols=1):
    """
    鲁棒读取 TDR txt 文件。
    只提取每行中的纯数字字段，自动跳过 CB323 等字符串字段。
    """
...
def build_twt_from_checkshot_drift(
    ...
    # ==========================================
    if z_start >= cs_z[0]:
        # 【情况 1】测井首点位于 checkshot 首点之后。
        if z_start <= cs_z[-1]:
            # np.interp 同时正确处理 z_start == cs_z[0] 的边界情况，
            # 避免原实现 idx_prev = -1 错取最后一个 Check-shot 点。
            t_start = float(np.interp(z_start, cs_z, cs_t))
        else:
...
def build_velocity_from_corrected_twt(
    depth,
    v_reference,
    twt_corrected,
    ac_unit="us/ft",
    velocity_bounds=(800.0, 8000.0),
):
    """
    根据 TDR/Check-shot 校正后的 TWT(z)，反推出与之相容的速度和 AC 曲线。

    离散关系与 build_twt_from_checkshot_drift 保持一致：
        dTWT_i = 2 * dz_i / V_i
        alpha_i = dTWT_corrected_i / dTWT_reference_i
        V_corrected_i = V_reference_i / alpha_i

    这里不对 alpha 或速度做静默裁剪，因为静默裁剪会破坏 TDR 闭合。
    若得到非物理速度，直接报错，提示检查 TDR 单位、深度基准或插值结果。

    返回
    ----
    v_corrected : ndarray
        TDR 校正后的速度，m/s。
    ac_corrected : ndarray
        与 v_corrected 对应的声波时差，单位由 ac_unit 决定。
    alpha : ndarray
        局部慢度比例；alpha > 1 表示原速度偏快，需要降低速度。
    twt_rebuilt : ndarray
        用 v_corrected 重新积分得到的 TWT，用于闭合 QC。
    closure_error : ndarray
        twt_rebuilt - twt_corrected，单位 s。
    """
    depth = np.asarray(depth, dtype=float).ravel()
    v_reference = np.asarray(v_reference, dtype=float).ravel()
    twt_corrected = np.asarray(twt_corrected, dtype=float).ravel()

    if not (len(depth) == len(v_reference) == len(twt_corrected)):
        raise ValueError("depth、v_reference 和 twt_corrected 长度必须一致。")
    if len(depth) < 2:
        raise ValueError("深度样点少于 2，无法反推速度。")
    if np.any(~np.isfinite(depth)) or np.any(np.diff(depth) <= 0):
        raise ValueError("深度轴必须有限且严格递增。")
    if np.any(~np.isfinite(v_reference)) or np.any(v_reference <= 0):
        raise ValueError("参考速度必须为有限正数。")

    dz_interval = np.diff(depth)
    dt_corr_interval = np.diff(twt_corrected)

    if np.any(~np.isfinite(dt_corr_interval)) or np.any(dt_corr_interval <= 0):
        raise ValueError(
            "TDR 校正后的 TWT 不是严格递增，不能据此构建物理速度模型。"
            "请检查 TDR/Check-shot 数据、MD/TVD 深度基准和漂移插值。"
        )

    # 与现有 TWT 积分代码保持同一离散约定：第 i 个时间增量使用 v[i]。
    dt_ref_interval = 2.0 * dz_interval / v_reference[1:]
    alpha_interval = dt_corr_interval / dt_ref_interval

    v_corrected = np.empty_like(v_reference)
    v_corrected[1:] = v_reference[1:] / alpha_interval
    v_corrected[0] = v_corrected[1]

    vmin, vmax = velocity_bounds
    if np.any(~np.isfinite(v_corrected)):
        raise ValueError("TDR 速度校正产生 NaN/Inf。")
    if np.min(v_corrected) < vmin or np.max(v_corrected) > vmax:
        raise ValueError(
            f"TDR 速度校正得到非预期速度范围 "
            f"{np.min(v_corrected):.1f}~{np.max(v_corrected):.1f} m/s，"
            f"超出安全检查范围 {vmin:.1f}~{vmax:.1f} m/s。"
            "请优先检查时间单位、TWT/OWT 类型以及 MD/TVD 基准，不建议直接裁剪。"
        )

    alpha = np.empty_like(v_reference)
    alpha[1:] = alpha_interval
    alpha[0] = alpha_interval[0]

    if ac_unit.lower() in ["us/ft", "us_per_ft", "usft"]:
        ac_corrected = 304800.0 / v_corrected
    elif ac_unit.lower() in ["us/m", "us_per_m", "usm"]:
        ac_corrected = 1e6 / v_corrected
    else:
        raise ValueError("ac_unit 只能是 'us/ft' 或 'us/m'。")

    twt_rebuilt = np.empty_like(twt_corrected)
    twt_rebuilt[0] = twt_corrected[0]
    twt_rebuilt[1:] = twt_corrected[0] + np.cumsum(
        2.0 * dz_interval / v_corrected[1:]
    )
    closure_error = twt_rebuilt - twt_corrected

    return v_corrected, ac_corrected, alpha, twt_rebuilt, closure_error
...
def run_data_preprocess(
    ...
    apply_tdr_velocity_correction=False,
    tdr_velocity_bounds=(800.0, 8000.0),
):
    ...
    print(f"  apply_tdr_velocity_correction = {apply_tdr_velocity_correction}")
    print(f"  tdr_velocity_bounds    = {tdr_velocity_bounds}")

    well_data = np.load(well_path)
    depth_raw = well_data[:, 0]
    ac_raw = well_data[:, 1]              # 之前保存的是声波时差 (AC)
    
    # 鲁棒兼容没有密度曲线的测井数据（例如 CB323）
    if well_data.shape[1] >= 3:
        rho_raw = well_data[:, 2]         # 之前保存的密度数据，单位 kg/m^3
        rho_is_gardner = False
    else:
    ...
    # tdr = robust_load_tdr_txt(r"D:\python_code\python_project\seismic_well\true_data_tying\data\initial_model_data\seismic\CB323_newTDR2.txt")
    # =========================================================
    # 1. 测井预处理
    # =========================================================
    ...
    # 使用清洗后的原始声波速度 v_sonic 进行第一遍漂移校正。
    twt_first_pass, twt_sonic_first_pass, drift_first_pass, drift_cs_first_pass = build_twt_from_checkshot_drift(
        depth=depth,
        v_eff=v_sonic, # 此时还没有 v_eff，直接用清洗后的 v_sonic
        checkshot_depth=checkshot_depth,
        checkshot_twt=checkshot_twt
    )

    # 保留原始清洗结果，确保新增功能可关闭、可对比。
    v_sonic_uncorrected = v_sonic.copy()
    rho_uncorrected = rho.copy()
    v_sonic_model = v_sonic.copy()
    rho_model = rho.copy()
    ac_tdr_corrected = np.full_like(v_sonic, np.nan)
    alpha_sonic_tdr = np.ones_like(v_sonic)
    twt_sonic_tdr_rebuilt = twt_first_pass.copy()
    twt_sonic_tdr_closure_error = np.zeros_like(twt_first_pass)

    if apply_tdr_velocity_correction:
        v_sonic_model, ac_tdr_corrected, alpha_sonic_tdr, twt_sonic_tdr_rebuilt, twt_sonic_tdr_closure_error = (
            build_velocity_from_corrected_twt(
                depth=depth,
                v_reference=v_sonic_uncorrected,
                twt_corrected=twt_first_pass,
                ac_unit=ac_unit,
                velocity_bounds=tdr_velocity_bounds,
            )
        )

        # 只有密度原本由 Gardner 公式估算时，才随校正速度重新估算。
        # 实测密度应保持不变，避免把 TDR 误当成密度校正信息。
        if rho_is_gardner:
            rho_model = 310.0 * (v_sonic_model ** 0.25)
            rho_model = np.clip(rho_model, 1500.0, 3500.0)
            rho_model = median_filter(rho_model, size=5, mode="nearest")

        print("[TDR 速度模型校正：声波曲线阶段]")
        print(f"  Vp before = {np.min(v_sonic_uncorrected):.1f} ~ {np.max(v_sonic_uncorrected):.1f} m/s")
        print(f"  Vp after  = {np.min(v_sonic_model):.1f} ~ {np.max(v_sonic_model):.1f} m/s")
        print(f"  alpha     = {np.min(alpha_sonic_tdr):.4f} ~ {np.max(alpha_sonic_tdr):.4f}")
        print(f"  max TWT closure error = {np.max(np.abs(twt_sonic_tdr_closure_error))*1000.0:.6f} ms")
    else:
        print("[TDR 速度模型校正] 未启用：保持原有流程，仅校正 TWT 映射。")

    # 此时获取井顶、井底时间，用于主频估计窗口。
    ...
    # =========================================================
    # 6. Check-shot 校正 TWT(z)，并可选反推最终 FD 速度模型
    # =========================================================
    # checkshot_depth, checkshot_twt = load_checkshot_time_md(
    #     checkshot_path,
    #     time_unit="ms",
...
#drift_cs：最终基于 Checkshot 得到的、更精细的时差校正曲线（包含高频成分）
    ...
    # Backus 粗化会轻微改变积分旅行时。启用时，再对最终 v_eff 做一次残差校正，
    # 使真正送入反射系数计算和 FD 正演的速度模型与 twt_init 一致。
    ac_eff_tdr_corrected = np.full_like(v_eff, np.nan)
    alpha_eff_tdr = np.ones_like(v_eff)
    twt_eff_tdr_rebuilt = twt_init.copy()
    twt_eff_tdr_closure_error = np.zeros_like(twt_init)

    if apply_tdr_velocity_correction:
        v_eff, ac_eff_tdr_corrected, alpha_eff_tdr, twt_eff_tdr_rebuilt, twt_eff_tdr_closure_error = (
            build_velocity_from_corrected_twt(
                depth=depth,
                v_reference=v_eff_uncorrected,
                twt_corrected=twt_init,
                ac_unit=ac_unit,
                velocity_bounds=tdr_velocity_bounds,
            )
        )
        print("[TDR 速度模型校正：最终 Backus/FD 模型]")
        print(f"  Vp before = {np.min(v_eff_uncorrected):.1f} ~ {np.max(v_eff_uncorrected):.1f} m/s")
        print(f"  Vp after  = {np.min(v_eff):.1f} ~ {np.max(v_eff):.1f} m/s")
        print(f"  alpha     = {np.min(alpha_eff_tdr):.4f} ~ {np.max(alpha_eff_tdr):.4f}")
        print(f"  max TWT closure error = {np.max(np.abs(twt_eff_tdr_closure_error))*1000.0:.6f} ms")
    ...
    np.savez(
        ...
        # cleaned logs
        depth=depth,
        v_sonic=v_sonic,                       # 保持原键语义：清洗后的原始声波 Vp
        v_sonic_uncorrected=v_sonic_uncorrected,
        v_sonic_tdr_corrected=v_sonic_model,
        ac_tdr_corrected=ac_tdr_corrected,
        alpha_sonic_tdr=alpha_sonic_tdr,
        rho=rho,                               # 保持原始清洗/估算密度
        rho_uncorrected=rho_uncorrected,
        rho_model_used=rho_model,
        rho_is_gardner=rho_is_gardner,

        # Backus / final FD model
        v_eff=v_eff,                           # 下游继续读取原键；启用时自动得到校正模型
        v_eff_uncorrected=v_eff_uncorrected,
        ac_eff_tdr_corrected=ac_eff_tdr_corrected,
        alpha_eff_tdr=alpha_eff_tdr,
        rho_eff=rho_eff,
        bulk_modulus_eff=rho_eff * (v_eff ** 2),
        ...
        drift_cs=drift_cs,
        twt_sonic_first_pass=twt_sonic_first_pass,
        drift_first_pass=drift_first_pass,
        drift_cs_first_pass=drift_cs_first_pass,
        twt_sonic_tdr_rebuilt=twt_sonic_tdr_rebuilt,
        twt_sonic_tdr_closure_error=twt_sonic_tdr_closure_error,
        twt_eff_tdr_rebuilt=twt_eff_tdr_rebuilt,
        twt_eff_tdr_closure_error=twt_eff_tdr_closure_error,
        tdr_velocity_correction_enabled=apply_tdr_velocity_correction,
    ...
    # 速度
    axes[0].plot(v_sonic_uncorrected, depth, label="clean Vp before TDR", linewidth=0.8)
    if apply_tdr_velocity_correction:
        axes[0].plot(v_sonic_model, depth, label="TDR-corrected sonic Vp", linewidth=0.9)
        axes[0].plot(v_eff_uncorrected, depth, label="Backus Vp before final correction", linewidth=1.0)
        axes[0].plot(v_eff, depth, label="final TDR-corrected FD Vp", linewidth=1.3)
    else:
    ...
    plt.savefig("F:\\01JDX_code\\python_code\\seismic_well_tying\\true_datacode_student_t_final_CB323\\_experiment_results\\Preprocess_Data\\fig_preprocess_qc.png", dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("F:\\01JDX_code\\python_code\\seismic_well_tying\\true_datacode_student_t_final_CB323\\_experiment_results\\Preprocess_Data\\fig_twt_checkshot_qc.png", dpi=300)
    plt.show()
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("F:\\01JDX_code\\python_code\\seismic_well_tying\\true_datacode_student_t_final_CB323\\_experiment_results\\Preprocess_Data\\fig_checkshot_drift_ms.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    # =========================================================
    well_path = r"F:\01JDX_code\python_code\seismic_well_tying\true_datacode_student_t_final_CB323\_experiment_data\well_data\well_data_CB323.npy"
    seismic_path = r"F:\01JDX_code\python_code\seismic_well_tying\true_datacode_student_t_final_CB323\_experiment_data\seismic_CB323\seismic_data_CB323_lowpass_70Hz.npz"
    tdr_path = r"C:\Users\15812\Desktop\程序打包\程序打包\一维声波正演程序\CB323.txt"
    checkshot_path = tdr_path
    output_npz_path = r"F:\01JDX_code\python_code\seismic_well_tying\true_datacode_student_t_final_CB323\_experiment_data\initial_model_data\initial_real_model_data_CB323.npz"
    ...
    # False：完全保持原流程，只用 TDR/Check-shot 校正 TWT 映射。
    # True ：额外将 TDR 校正写入声波 Vp 与最终 FD 速度模型。
    apply_tdr_velocity_correction = True
    tdr_velocity_bounds = (800.0, 8000.0)
    ...
    run_data_preprocess(
        ...
        apply_tdr_velocity_correction=apply_tdr_velocity_correction,
        tdr_velocity_bounds=tdr_velocity_bounds,
    )
