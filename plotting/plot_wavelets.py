...
def _nearest_indices_with_reliable_priority(
    ...
    for idx_raw, target_t in zip(raw_indices, target_times_s):
        ...
        if reliable_mask[idx_raw]:
            selected_indices.append(idx_raw)
            continue
        # 诊断图优先挑选最近的 reliable 时间，避免把非直接反演中心误当代表性子波。
...
# valid_mask 在本流程中表示“该时刻是否为直接反演中心”；它不等于最终子波 QC。
# 判断指定索引的子波状态是 direct-inverted、reliable、fallback。
...
def plot_wavelet_selected_time_comparison(
    *,
    ...
    # Validate optional mask/metric vectors
    def _validate(val, name, is_bool=True):
        func = _optional_bool_vector if is_bool else _optional_float_vector
        return func(val, name=name, n_time=n_time)

    valid_mask = _validate(valid_mask, "valid_mask")
    reliable_mask = _validate(reliable_mask, "reliable_mask")
    fallback_mask = _validate(fallback_mask, "fallback_mask")
    fallback_alpha = _validate(fallback_alpha, "fallback_alpha", is_bool=False)
    peak_metric_ms = _validate(peak_metric_ms, "peak_metric_ms", is_bool=False)
    wavelet_energy_norm = _validate(wavelet_energy_norm, "wavelet_energy_norm", is_bool=False)
    side_lobe_ratio = _validate(side_lobe_ratio, "side_lobe_ratio", is_bool=False)
    edge_energy_ratio = _validate(edge_energy_ratio, "edge_energy_ratio", is_bool=False)
    ...
    for ip, idx in enumerate(target_indices):
        ...
        if valid_mask is not None and not bool(valid_mask[idx]):
            # 灰底仅表示该时刻不是直接反演中心，不表示最终子波未通过 QC。
...
def plot_time_varying_wavelet_wiggle_panel(
    *,
    ...
    for irow, idx in enumerate(target_indices):
        ...
        t_center = float(t_work[idx_raw])
        x_axis = t_center + lag_s
        y0 = row_offsets[irow]

        if valid_mask is not None and not bool(valid_mask[idx]):
            # 灰带仅表示该时刻不是直接反演中心，不表示最终子波未通过 QC。
